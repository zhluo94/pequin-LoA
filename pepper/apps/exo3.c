...
void compute(int *input, uint8_t **output, uint32_t* session_idxes) ;
...
int main(int argc, char **argv) {
    ...
    uint32_t* session_idxes = calloc(output_length, sizeof(uint32_t));

    compute(input, output, session_idxes);
...
void compute(int *input, uint8_t **output, uint32_t* session_idxes) {
    ...
    int total_num_sessions = input[1];
    int i, j;
